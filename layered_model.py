...
# This funciton will define a neural netwoek model with a single dense layer. A dense layer is a fully connected neural network layer, where each input node is connected to each output node.
...
def fit_mnist_model_single_digit(
    x_train, y_train, target_digit, model, epochs=10, batch_size=128
):
